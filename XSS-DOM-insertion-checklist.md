...
**Yardımcılar (js/utils.js):** `sanitizeHTML`, `safeAttr`, `renderEmptyState(message)`, `renderCategoryItem(categoryName, balance, status)`, `renderTransactionHistoryItem(t)` (içeride sanitize/kaçış kullanır).
